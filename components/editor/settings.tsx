...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  return <SettingsContext.Provider value={{ keys, model, setKey, setModel }}>{children}</SettingsContext.Provider>
}
                      ...
                      <CommandList>
                        <CommandGroup>
                          {models.map((m) => (
                            ...
                            >
                              <Check
                                className={cn("mr-2 size-4", model.value === m.value ? "opacity-100" : "opacity-0")}
                              />
                              <code>{m.label}</code>
                            </CommandItem>
                          ))}
